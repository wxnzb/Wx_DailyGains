...
g0不是用户创建的，是每个线程都有的一个调度携程，是runtime的核心部分，在线程上要是由携程因为i/o等发生了阻塞，那么g0会将这个线程和处理器进行解绑操作，然后让线程处于阻塞状态不占用cpu的资源，然后将这个处理器加入到全局处理器中，其他空闲线程就可以于这个处理器进行绑定，然后去执行它上面的其他携程，要是阻塞的携程恢复了，那么g0会在去全局队列里面取一个处理器,与线程绑定
...
细节：**他问你map的底层实现你应该怎么回答**

map的底层是哈希表，具体采用的就是树组+链表的形式；它主要有两个重要结构体：hmap和bmap;hmap是哈希表的整体结构，有`buckets` 数组，bmap桶存储实际的键值对；每个bmap最多存储8个键值对，他里面有个tophash,tophash存储的是每个键值对的高8位，这是查找是优化的关键，查找时会先对比tophash,不一致就直接跳过，减少完整键的比较次数

插入查找：先对key进行哈希计算，然后将哈希值低x位决定key属于哪个桶，哈希值高8位存储在tophash中
...
Go 协程（goroutine）之间是 **怎样共享与通信的**？channel

**携程上下文切换做了什么？**

首先会保存当前携程的上下文：程序计数器，栈信息，还有少量寄存器数据；然后调度器会找到下一个携程，把这个携程之前保存的状态加载到cpu。他的整个过程都是在用户态完成不需要内核太的参加，因此比线程切换开销小。
